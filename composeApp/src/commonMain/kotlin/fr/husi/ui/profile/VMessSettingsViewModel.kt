...
internal class VMessSettingsViewModel : StandardV2RaySettingsViewModel<VMessBean>() {
    ...
    override val uiState: StateFlow<VMessUiState>
        field = MutableStateFlow(VMessUiState())
    ...
    fun setAlterID(alertID: Int) {
        uiState.update { it.copy(alterID = alertID) }
    }
    ...
    fun setAuthenticatedLength(enabled: Boolean) {
        uiState.update { it.copy(authenticatedLength = enabled) }
    }
