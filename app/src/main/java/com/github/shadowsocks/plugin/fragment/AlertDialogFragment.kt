 ...
 */
abstract class AlertDialogFragment<Arg : Parcelable, Ret : Parcelable?>(val argClass: Class<Arg>) :
    AppCompatDialogFragment(), DialogInterface.OnClickListener {
    companion object {
        private const val KEY_RESULT = "result"
        private const val KEY_ARG = "arg"
        const val KEY_RET = "ret"
        const val KEY_WHICH = "which"

        inline fun <reified Ret : Parcelable> setResultListener(
            fragment: Fragment, requestKey: String,
            crossinline listener: (Int, Ret?) -> Unit
        ) {
            fragment.setFragmentResultListener(requestKey) { _, bundle ->
                listener(
                    bundle.getInt(KEY_WHICH, Activity.RESULT_CANCELED),
                    BundleCompat.getParcelable(bundle, KEY_RET, Ret::class.java)
                )
            }
        }
    ...
    protected val arg by lazy {
        ...
        BundleCompat.getParcelable(arguments, KEY_ARG, argClass)!!
    }
